...
\begin{abstract}
  ...
  \marginnote{Do we need this statement in the abstract?
  Further, our solver is able to solve a
...
benchmarks satisfactorily.
We observed that the anytime extension of the algorithm was able to
produce more compact solutions in $22$ cases.
We were able to obtain upto $90\%$ reduction in solution size in some
cases.
\comment{
 The tool was able
...
\begin{subfigure}{0.33\textwidth}
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      \addplot[mark=.,color=red] table {plots/general_eusolver.dat};
      \addplot[mark=.,color=blue] table {plots/general_cvc4.dat};
      \addplot[mark=.,color=green] table {plots/general_esolver.dat};
    \end{axis}
    %\end{semilogyaxis}
  \end{tikzpicture}
  \caption{General track.}
...
\begin{subfigure}{0.33\textwidth}
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      \addplot[mark=.,color=red] table {plots/lia_eusolver.dat};
      \addplot[mark=.,color=blue] table {plots/lia_cvc4.dat};
    \end{axis}
    %\end{semilogyaxis}
  \end{tikzpicture}
  \caption{LIA track.}
...
\begin{subfigure}{0.32\textwidth}
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      \addplot[mark=.,color=red] table {plots/pbe_eusolver.dat};
      \addplot[mark=.,color=blue] table {plots/pbe_cvc4.dat};
    \end{axis}
    %\end{semilogyaxis}
  \end{tikzpicture}
  \caption{PBE track.}
...
\centering
  ...
  $y$-coordinate.
  \caption{Results for \esolver (green), CVC4 (blue), and \eusolver (red)}
  \label{fig:results}
\end{figure}
...
Figure~\ref{fig:results} plots the full results of running \eusolver on
the benchmarks from the three categories.
Alongside are plotted the results of $2$ other state-of-the-art
solvers:
\begin{inparaenum}[(a)]
\item the white-box solver CVC4-1.5.1 based on~\cite{reynolds-15}, and
\item the enumerative black-box solvers \esolver described in~\cite{udupa-sygus}
\end{inparaenum}

\paragraph{Performance}
\eusolver was able to solve $206$ of the $309$ benchmarks in the
general track and $72$ of the $73$ benchmarks in the
programming-by-example track. 
CVC4-1.5.1 solves $195$ and $73$ benchmarks in these categories, while 
\esolver solves $139$ and $XXX$ \arsays{rerun esolver on lia}.
As Figure~\ref{fig:results} shows, the performance is comparable to both
CVC4-1.5.1 and \esolver in both tracks, being only marginally slower in
the LIA track.
However, \eusolver performs exceptionally well on the PBE benchmarks,
solving $787$ while CVC4-1.5.1 solved $39$ and \esolver solved $1$.
% As the results in Figure~\ref{fig:results} show,
Programming-by-examples benchmarks require the solver
to generalize from input-output examples---\eusolver inherits this from
the enumerative approach.
However, the standard enumerative solver \esolver is unable to solve
these benchmarks as the solution sizes are large---\eusolver overcomes
...
\marginnote{How to write this? Almost all the commonly solved benchmarks
have equal solution sizes}
Figure~\ref{fig:sizes} highlights the solution sizes produced by the
three solvers for the commorly solved benchmarks.
As can be seen, \eusolver often matches the solution sizes produced by
$\esolver$ which is guaranteed to produce the smallest solution
possible.
This shows that the divide-and-conquer approach
does not significantly sacrifice solution quality for better
performance. 

\begin{wrapfigure}{l}{0.4\textwidth}%[!t]
\centering
  \begin{tikzpicture}
    \fontsize{8}{10}\selectfont
    \begin{loglogaxis}[
  ...
  \caption{Scatter plot of first vs. minimum size
    solutions with the anytime extension. Points below the $x = y$ line
    correspond to benchmarks which benefit from the anytime extension.}
...
enumerated predicates using decision trees. 
We have demonstrated the performance and scalability of the algorithm by
evaluating it on standard benchmarks, with exceptional performance on
programming-by-example benchmarks.
% We have demonstrated the ability of the algorithm to generalize from
% input-output examples by solving most of the previously unsolved ICFP
% benchmarks in the \sygus benchmark suite.
The algorithm is generic, efficient,
