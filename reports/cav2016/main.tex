...
\begin{example}
  ...
  learns the decision tree corresponding to the expression
  $\mathtt{if}~x \leq y~\mathtt{then}~0~\mathtt{else}~1$.
  Now, verifying this expression produces a counter-example point, say
  $\{ x \gets 1, y \gets 2 \}$.
  While the term $0$ is incorrect on this point, the term $y$ which was
  discarded as an equivalent term to $0$, is correct.
\end{example}

Hence, for a practical implementation of the algorithm we do not discard
these terms and predicates, but store them separately as a equivalence
relation.
Formally, at lines~\ref{line:main:xxx},~\ref{line:main:yyy},
and~\line{line:main:zzz}, if the check for distinctness fails, we
instead add the term $\Term$ to a $\Equiv$ map\dots.
Now, when the decision tree learning algorithm returns a f
