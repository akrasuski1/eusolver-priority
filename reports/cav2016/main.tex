...
\item black-box solvers, which do not deal with the specification
  ...
  behave on sample inputs points~\cite{udupa-transit,udupa-sygus,saha-15};
  and
\item white-box solvers, which attempt to solve the specification
  directly using constraint solving
...
\begin{subfigure}{0.45\textwidth}
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Total number of benchmarks solved as a function of elapsed time}
...
\begin{subfigure}{0.45\textwidth}
  \begin{tikzpicture}
    ...
    \end{loglogaxis}
  \end{tikzpicture}
  \caption{Scatter plot of first vs. minimum size solutions}
...
minutes, as seen in Figure~\ref{subfigure:solved_vs_time} indicates
this, with the black line denoting the time to find the first
solution, and the red line denoting the time to find the best
solution.
...
% \pwsolve uses a different set of heuristics
...
\item
  ...
  struggle to do.
  % often representing a constant like ``42'' as a sum of
  % smaller constants that appear in the grammar.
\item
...
% The algorithm produces compact solutions,
