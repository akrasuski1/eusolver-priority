...
\begin{table}[!t]
  ...
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}clcc}\\\hlx{hv}
      ...
      3 & $0$, $1$, $x$, $y$, \st{$0 + x$}, \ldots, $x + y$,  & $x+y$ & $\{ x \mapsto 1, y \mapsto 2 \}$ \\\hlx{h}
...
the \textsc{enumerate} procedure which returns a (possibly infinite)
list of expressions such that no two expressions in this list evaluate
to the same values at every point $\Point \in \Points$
(line~\ref{line:basic:enumerate}).  Every expression $\Expr$ in this
list is then verified, first on the set of points
