...
Consider a synthesis task to generate an expression such that:
\begin{inparaenum}[(a)]
...
\end{inparaenum}
Note that the specification constrains $\SynthFun(x, y)$ to be the
maximum of the two inputs $x$ and $y$.
...
predicates.
The enumeration of predicates proceeds until we find a sufficient number
of predicates to generate a conditional expression using the previously
enumerated terms.
