...
There are several key similarities and differences between this
algorithm and Algorithm~\ref{algo:basic}.
\begin{compactitem}
\item As in Algorithm~\ref{algo:basic}, the divide-and-conquer algorithm
  maintains a set of points $\Points$ to guide the enumeration.
  When a potential solution is verified and found to be incorrect, the
  corresponding counter-example point is added to $\Points$
  (lines~\ref{line:main:xxx}-\ref{line:main:yyy}).
\item In Algorithm~\ref{algo:basic}, the enumeration continues until we
  find one expression that is correct for all the points in $\Points$. 
  However, here, the enumeration continues only until for each point
  in $\Points$, there is at least one expression that is correct on it.
  \arsays{We only put them exprs if they are distinct with respect to
  points on spec}
\item 
\end{compactitem}
...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \While { $\True$ }
    \State $\Expressions \gets \emptyset; \Predicates \gets \emptyset$
    ...
    \EndIf
    \State $\Expr \gets \DTtoExpr{\DecisionTree}$
    \EndWhile

    \State $\CexInput \gets \Verify(\Expr, \Spec)$ \Comment { Verifier }
    \If { $\CexInput = \bot$ } \Return $\Expr$ \EndIf 
    \State $\Points \gets \Points \cup \CexInput$ 
    \EndWhile
  \end{algorithmic}
